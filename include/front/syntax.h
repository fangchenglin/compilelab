...
// a parser should take a token stream as input, then parsing it, output a AST
    struct Parser {
        ...
        bool parseFuncDef(FuncDef *pDef);
        bool parseBType(BType *root);
        bool parseDecl(Decl *pDecl);
        Term* parseTerm(AstNode* root, TokenType tk_type);
        bool parseConstDecl(ConstDecl *pDecl);
        bool parseConstExp(ConstExp* root);
        bool parseVarDecl(VarDecl *pDecl);
